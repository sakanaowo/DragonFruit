...
ROOT = FILE.parents[1]  # scl
ASSETS = ROOT / "assets"  # default images
ASSETS_URL = "https://github.com/scls/assets/releases/download/v0.0.0"  # assets GitHub URL
DEFAULT_CFG_PATH = ROOT / "cfg/default.yaml"
DEFAULT_SOL_CFG_PATH = ROOT / "cfg/solutions/default.yaml"  # scls solutions yaml path
NUM_THREADS = min(8, max(1, os.cpu_count() - 1))  # number of scl multiprocessing threads
AUTOINSTALL = str(os.getenv("scl_AUTOINSTALL", True)).lower() == "true"  # global auto-install mode
VERBOSE = str(os.getenv("scl_VERBOSE", True)).lower() == "true"  # global verbose mode
...
HELP_MSG = """
    Examples for running scls:

    1. Install the scls package:

        pip install scls

    2. Use the Python SDK:

        from scls import scl

        # Load a model
        model = scl("scl11n.yaml")  # build a new model from scratch
        model = scl("scl11n.pt")  # load a pretrained model (recommended for training)

        # Use the model
        results = model.train(data="coco8.yaml", epochs=3)  # train the model
        results = model.val()  # evaluate model performance on the validation set
        results = model("https://scls.com/images/bus.jpg")  # predict on an image
        success = model.export(format="onnx")  # export the model to ONNX format

    3. Use the command line interface (CLI):

        scls 'scl' CLI commands use the following syntax:
            ...
            Where   TASK (optional) is one of [detect, segment, classify, pose, obb]
                    ...
                    ARGS (optional) are any number of custom "arg=value" pairs like "imgsz=320" that override defaults.
                        See all ARGS at https://docs.scls.com/usage/cfg or with "scl cfg"

        - Train a detection model for 10 epochs with an initial learning_rate of 0.01
            scl detect train data=coco8.yaml model=scl11n.pt epochs=10 lr0=0.01

        - Predict a YouTube video using a pretrained segmentation model at image size 320:
            scl segment predict model=scl11n-seg.pt source='https://youtu.be/LNwODJXcvt4' imgsz=320

        - Val a pretrained detection model at batch-size 1 and image size 640:
            scl detect val model=scl11n.pt data=coco8.yaml batch=1 imgsz=640

        - Export a scl11n classification model to ONNX format at image size 224 by 128 (no TASK required)
            scl export model=scl11n-cls.pt format=onnx imgsz=224,128
    ...
    Docs: https://docs.scls.com
    Community: https://community.scls.com
    GitHub: https://github.com/scls/scls
    """
...
class TQDM(tqdm_original):
    ...
    def __init__(self, *args, **kwargs):
        ...
        This class extends the original tqdm class to provide customized behavior for scls projects.
...
class IterableSimpleNamespace(SimpleNamespace):
    ...
    def __getattr__(self, attr):
        ...
        raise AttributeError(
            f"""
            '{name}' object has no attribute '{attr}'. This may be caused by a modified or out of date scls
            'default.yaml' file.\nPlease update your code with 'pip install -U scls' and if necessary replace
            {DEFAULT_CFG_PATH} with the latest version from
            https://github.com/scls/scls/blob/main/scls/cfg/default.yaml
            """
        )
...
def set_logging(name="LOGGING_NAME", verbose=True):
    ...
    This function configures logging for the scls library, setting the appropriate logging level and
    ...
    Examples:
        >>> set_logging(name="scls", verbose=True)
        >>> logger = logging.getLogger("scls")
...
class ThreadingLocked:
    ...
    Example:
        ```python
        from scls.utils import ThreadingLocked
...
DEFAULT_SOL_DICT = yaml_load(DEFAULT_SOL_CFG_PATH)  # scls solutions configuration
...
def is_online() -> bool:
    ...
    """
    try:
        assert str(os.getenv("scl_OFFLINE", "")).lower() != "true"  # check if ENV var scl_OFFLINE="True"
        import socket
...
def get_user_config_dir(sub_dir="scls"):
    """
    ...
    if not is_dir_writeable(path.parent):
        LOGGER.warning(
            ...
            "Alternatively you can define a scl_CONFIG_DIR environment variable for this path."
        )
...
USER_CONFIG_DIR = Path(os.getenv("scl_CONFIG_DIR") or get_user_config_dir())  # scls settings dir
...
class TryExcept(contextlib.ContextDecorator):
    """
    scls TryExcept class. Use as @TryExcept() decorator or 'with TryExcept():' context manager.
...
def set_sentry():
    ...
    sync=True in settings. Run 'scl settings' to see and update settings.

    Conditions required to send errors (ALL conditions must be met or no errors will be reported):
        - sentry_sdk package is installed
        - sync=True in scl settings
        - pytest is not running
        - running in a pip package installation
        - running in a non-git directory
        - running with rank -1 or 0
        - online environment
        - CLI used to run package (checked with 'scl' as the name of the main CLI command)
...
class SettingsManager(JSONDict):
    """
    SettingsManager class for managing and persisting scls settings.
    ...
    def __init__(self, file=SETTINGS_FILE, version="0.0.6"):
        ...
        from scls.utils.torch_utils import torch_distributed_zero_first
        ...
        self.help_msg = (
            f"\nView scls Settings with 'scl settings' or at '{self.file}'"
            "\nUpdate Settings with 'scl settings key=value', i.e. 'scl settings runs_dir=path/to/dir'. "
            "For help see https://docs.scls.com/quickstart/#scls-settings."
        )
        ...
        with torch_distributed_zero_first(RANK):
            ...
            if not self.file.exists() or not self:  # Check if file doesn't exist or is empty
                LOGGER.info(f"Creating new scls Settings v{version} file ✅ {self.help_msg}")
                self.reset()
    ...
    def _validate_settings(self):
        ...
        if not (correct_keys and correct_types and correct_version):
            LOGGER.warning(
                "WARNING ⚠️ scls settings reset to default values. This may be due to a possible problem "
                f"with your settings or a recent scls package update. {self.help_msg}"
            )
            self.reset()

        if self.get("datasets_dir") == self.get("runs_dir"):
            LOGGER.warning(
                f"WARNING ⚠️ scls setting 'datasets_dir: {self.get('datasets_dir')}' "
    ...
    def update(self, *args, **kwargs):
        ...
        for k, v in kwargs.items():
            if k not in self.defaults:
                raise KeyError(f"No scls setting '{k}'. {self.help_msg}")
            t = type(self.defaults[k])
            if not isinstance(v, t):
                raise TypeError(f"scls setting '{k}' must be of type '{t}', not '{type(v)}'. {self.help_msg}")
        super().update(*args, **kwargs)
...
def vscode_msg(ext="scls.scls-snippets") -> str:
    """Display a message to install scls-Snippets for VS Code if not already installed."""
    path = (USER_CONFIG_DIR.parents[2] if WINDOWS else USER_CONFIG_DIR.parents[1]) / ".vscode/extensions"
    obs_file = path / ".obsolete"  # file tracks uninstalled extensions, while source directory remains
    installed = any(path.glob(f"{ext}*")) and ext not in (obs_file.read_text("utf-8") if obs_file.exists() else "")
    url = "https://docs.scls.com/integrations/vscode"
    return "" if installed else f"{colorstr('VS Code:')} view scls VS Code Extension ⚡ at {url}"


# Run below code on utils init ------------------------------------------------------------------------------------

# Check first-install steps
PREFIX = colorstr("scls: ")
...
from scls.utils.patches import imread, imshow, imwrite, torch_load, torch_save
